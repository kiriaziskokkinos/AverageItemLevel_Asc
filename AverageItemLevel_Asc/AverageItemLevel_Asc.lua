...
local function GameTooltipOnEvent(self, event, ...)
	...
	if event == "INSPECT_TALENT_READY" then -- UPDATE ILVL if > 0 and different than cached
		...
		-- end
        AiL.updateCacheIlvl(unit)
	elseif event == "AIL_INSPECT_IN_PROGRESS" then
		for i = 1, self:NumLines() do
			local Ltext = _G["GameTooltipTextLeft" .. i]:GetText() or ""
			if string.match(Ltext, AiL.hiddenText) then -- looks for our hidden text
				if AiL.getCacheForUnit(unit).true_ilvl == -1 then                   
					if _G["GameTooltipTextRight" .. i]:GetText() == "(   )"  or _G["GameTooltipTextRight" .. i]:GetText() == AiL.getColoredIlvlString(0, -1)   then
						_G["GameTooltipTextRight" .. i]:SetText("(.  )")
					elseif _G["GameTooltipTextRight" .. i]:GetText() == "(.  )" then
						_G["GameTooltipTextRight" .. i]:SetText("(.. )")
					elseif _G["GameTooltipTextRight" .. i]:GetText() == "(.. )" then
						_G["GameTooltipTextRight" .. i]:SetText("(...)")
					elseif _G["GameTooltipTextRight" .. i]:GetText() == "(...)" then
	...
	elseif
		(event == "MYSTIC_ENCHANT_INSPECT_RESULT" and IsHeroClass(unit))
		or (event == "INSPECT_CHARACTER_ADVANCEMENT_RESULT" and select(1, ...) == "CA_INSPECT_OK")
	then
...
GameTooltip:RegisterEvent("AIL_INSPECT_IN_PROGRESS")
